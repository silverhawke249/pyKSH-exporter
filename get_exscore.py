...
class Parser():
    ...
    def get_active_bpm(self, timepoint: Tickcount) -> Decimal:
        prev_bpm = Decimal(0)
    ...
    def get_active_timesig(self, timepoint: Tickcount) -> TimeSignature:
    ...
    def get_active_tickrate(self, timepoint: Tickcount) -> int:
        return 24 if self.get_active_bpm(timepoint) >= 256 else 12
    ...
    def handle_line(self, line) -> None:
        ...
        timepoint = self.str_to_tick(time_str)
    ...
    def calculate_notes(self) -> None:
        for key, value in self.vox_data.buttons.items():
            # Chip notes
            if value == 0:
                self.chip_count += 1
                continue
            ...
            cur_timept = timepoint
            # Add ticks until it exceeds endpoint
            # TODO: Factor in BPM changes
            while cur_timept < timepoint + value:
                if cur_timept % tick_rate != 0:
        ...
        for laser_type, laser_timepts in self.vox_data.lasers.items():
            for laser_s, laser_e in zip(*[iter(laser_timepts)] * 2):
                ...
                # Round to next tick
                if cur_timept % tick_rate != 0:
                    cur_timept += tick_rate - (laser_s % tick_rate)
                # Add ticks until it exceeds endpoint
                # TODO: Factor in BPM changes
            ...
            for timepoint in slam_timepts:
                ...
                if timepoint in laser_ticks[laser_type]:
                    if timepoint in slam_overlapping_ticks[laser_type]:
                        ...
                        slam_overlapping_ticks[laser_type].append(timepoint)
                else:
                    self.laser_count += 1
